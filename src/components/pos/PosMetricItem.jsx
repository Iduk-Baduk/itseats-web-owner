...
const checkCookTime = (cookTime) => {
  ...
  const splitString = cookTime.split("분");
  const timeInMinutes = parseInt(splitString[0], 10);
  if (timeInMinutes <= 20) {
    return goodCheckIcon();
...
const checkAccuracy = (accuracy) => {
  ...
  const splitString = accuracy.split("%");
  const accuracyInPercentage = parseInt(splitString[0], 10);
...
const checkPickupTime = (pickupTime) => {
  ...
  const splitString = pickupTime.split("초");
  const timeInSeconds = parseInt(splitString[0], 10);
  if (timeInSeconds <= 30) {
    return goodCheckIcon();
...
const checkOrderAcceptanceRate = (orderAcceptanceRate) => {
  ...
  const splitString = orderAcceptanceRate.split("%");
  const rateInPercentage = parseInt(splitString[0], 10);
  ...
  return (
    <div className={styles.icon}>
      <svg
        ...
        xmlns="http://www.w3.org/2000/svg"
      >
        <rect x="0.65625" width="24" height="24" rx="12" fill="#FAC40E" />
        <path
          d="M10.656 14.7801L8.34269 12.4667C8.21804 12.3421 8.04897 12.272 7.87269 12.272C7.6964 12.272 7.52734 12.3421 7.40269 12.4667C7.27804 12.5914 7.20801 12.7604 7.20801 12.9367C7.20801 13.024 7.2252 13.1104 7.2586 13.1911C7.29201 13.2717 7.34097 13.345 7.40269 13.4067L10.1894 16.1934C10.4494 16.4534 10.8694 16.4534 11.1294 16.1934L18.1827 9.14005C18.3073 9.0154 18.3774 8.84634 18.3774 8.67005C18.3774 8.49377 18.3073 8.3247 18.1827 8.20005C18.058 8.0754 17.889 8.00537 17.7127 8.00537C17.5364 8.00537 17.3673 8.0754 17.2427 8.20005L10.656 14.7801Z"
          fill="white"
        />
